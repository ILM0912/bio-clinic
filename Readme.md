...
Приложение позволяет пациентам просматривать филиалы, медицинские услуги и врачей, создавать и удалять записи на приём, а также просматривать личный кабинет. Врач может просматривать расписание приёмов и отмечать завершённые посещения. Администратор управляет справочными данными через административную панель Django.

Демонстрационная версия проекта доступна по ссылке:
https://bio-clinic.duckdns.org
Приложение развёрнуто на удалённом сервере с использованием Docker Compose, доступно по доменному имени и защищено TLS-сертификатом.
...
- удаление записи пациента на приём;
...
Для проверки работы приложения предусмотрено заполнение тестовыми данными - филиалами, услугами и врачами.
 ...
 - Фикстуры базы данных расположены в файле `backend/clinic/fixtures/db.json
...
| DELETE | /api/appointments/{id}/             | Удаление записи пациентом                    |
| PATCH  | /api/appointments/{id}/             | Отметка завершения приёма врачом             |
